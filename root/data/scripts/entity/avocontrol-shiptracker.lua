  ...
  if using_stack_exploit then
    if e.allianceOwned then
      if messages_sent < messages_till_kick then
        ...
        local server = Server()
        for _, member in pairs(Alliance(e.factionIndex)) do
          if server:isOnline(member.factionIndex) then
            print("doPlayerKickEvent: ${p} ${r}"%_T % {
              p=member.factionIndex,
              r="(Alliance) Using known exploits despite being warned"})
          end
        end
      end
      ...
      Alliance(e.factionIndex):sendChatMessage("Server", ChatMessageType.Error,
        "The ship \"${n}\" currently has multiple unique artifacts of the same type installed.\n"%_T % {
          n=tostring(e.name)}..
        "This has been logged and the admins have been notified.\n"..
        "You will continue to get this message until they are removed, or you are auto-kicked.")
      return
    end
